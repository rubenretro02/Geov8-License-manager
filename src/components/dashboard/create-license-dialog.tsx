...
export function CreateLicenseDialog({ profile }: CreateLicenseDialogProps) {
  ...
  // Check if user is self-registered (admin_id equals their own id OR admin_id is null)
  const isSelfRegistered = profile?.role === 'user' &&
    (profile?.admin_id === profile?.id || !profile?.admin_id)
  // Max trial days: 1 for self-registered users, 5 for others
  const maxTrialDays = isSelfRegistered ? 1 : 5
  ...
  // Calculate credits cost (matching the calculateCreditsForDays function)
  const getCreditsCost = () => {
    if (formData.is_trial) return 0
    if (formData.is_permanent) return 300
    // 365 days = only 200 credits (44% discount)
    if (formData.days_valid === 365 || (formData.days_valid >= 360 && formData.days_valid <= 370)) {
      return 200
    }
            ...
            <div className="text-center space-y-2">
              ...
              {formData.auto_renew && (
                <p className="text-sm text-blue-400 flex items-center justify-center gap-1">
                  ...
                  Auto-renewal enabled ({formData.auto_renew_days} days)
                </p>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (formData.is_trial && value > maxTrialDays) value = maxTrialDays
                    if (value < 1) value = 1
                    setFormData({ ...formData, days_valid: value })
                  }}
                  min={1}
                  max={formData.is_trial ? maxTrialDays : undefined}
                  className="bg-zinc-800 border-zinc-700 text-white"
                />
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.is_trial ? (
                    Array.from({ length: maxTrialDays }, (_, i) => i + 1).map((d) => (
                      <Button
                        key={d}
                        type="button"
                        variant="outline"
            ...
            <div className="flex items-center justify-between p-4 bg-purple-500/10 border border-purple-500/20 rounded-xl">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  {t('freeTrialPeriod')} (max {maxTrialDays} day{maxTrialDays > 1 ? 's' : ''})
                  {isSelfRegistered && (
                    <span className="text-amber-400 ml-1">- 1 trial/month</span>
                  )}
                </p>
              </div>
              <Switch
                ...
                onCheckedChange={(checked) => setFormData({
                  ...
                  auto_renew: checked ? false : formData.auto_renew,
                  days_valid: checked ? Math.min(formData.days_valid || maxTrialDays, maxTrialDays) : formData.days_valid
                })}
              />
            </div>
              ...
              <div className="flex items-center justify-between p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                <div>
                  <p className="text-white font-medium flex items-center gap-2">
                    ...
                    {lang === 'es' ? 'Auto Renovar' : 'Auto Renew'}
                  </p>
                  <p className="text-xs text-zinc-500">
                    {lang === 'es'
                      ? `Renovar automáticamente usando ${formData.days_valid} créditos`
                      : `Auto-renew using ${formData.days_valid} credits when expired`}
                  </p>
                </div>
                <Switch
                  ...
                  onCheckedChange={(checked) => setFormData({
                    ...formData,
                    auto_renew: checked,
                    auto_renew_days: checked ? formData.days_valid : 30,
                  })}
                />
              </div>
            )}
                  ...
                  <span className="text-sm text-white">
                    {formData.is_trial
                      ...
                      : formData.is_permanent
                        ? 'Permanent License'
                        : formData.auto_renew
                          ? `${formData.days_valid}-Day License (Auto-renew)`
